...
pmi, _  = latest_value(data, "NAPM")       # ISM PMI composite (proxy for new orders)
...
tiles = [
    ...
    ("PMI (ISM)", pmi, lambda v: v >= TH["pmi_green_min"], lambda v: v >= TH["pmi_yellow_min"]),
...
st.caption("Data: FRED (no API) & NY Fed. USD tile uses FRED Broad Dollar Index (DTWEXBGS). Thresholds in config.yaml.")
...
def make_export():
    ...
    with pd.ExcelWriter(out, engine="openpyxl") as xls:
        ...
        latest_tbl = pd.DataFrame({
            "Indicator":["Y10","Y2","Y3M","TIPS","TP10","BROAD$","PMI (ISM)","HY OAS (bps)","10s-2s (bps)","WTI"],
